<script>
  ...
  function handleFileUpload(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        imported.forEach((t) => { t.category = categorize(t.description); });
  ...
  function addTransaction() {
    ...
    transactions = [...transactions, { date: dateStr, description: desc, amount, category, source: 'manual' }];
  }
...
<div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex items-center justify-between h-16">
        ...
        <div class="flex gap-2">
          <button class="bg-emerald-600 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-emerald-700 active:scale-95 transition-all text-sm sm:text-base font-medium" on:click={addTransaction}>
  ...
  <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24 lg:pb-6">
    {#if showImport}
      <div class="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg mb-6">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-3">Wähle eine CSV-Datei von deiner Bank aus (DKB, N26, Revolut, Comdirect)</p>
        <input type="file" accept=".csv" on:change={handleFileUpload} class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:bg-indigo-50 file:text-indigo-700 hover:file:bg-indigo-100 dark:file:bg-indigo-900 dark:file:text-indigo-300" />
        {#if importMessage}<p class="mt-2 text-sm text-gray-600 dark:text-gray-400">{importMessage}</p>{/if}
      </div>
    {/if}

    <div class="flex overflow-x-auto gap-1 mb-6 bg-white dark:bg-gray-800 p-1 rounded-xl shadow-sm lg:hidden no-scrollbar">
      <button class="flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-colors {activeTab === 'overview' ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}" on:click={() => activeTab = 'overview'}>Übersicht</button>
      <button class="flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium transition-colors {activeTab === 'budget' ? 'bg-indigo-600 text-white' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}" on:click={() => activeTab = 'budget'}>Budget</button>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
      <div class="bg-gradient-to-br from-green-400 to-green-600 rounded-2xl p-4 sm:p-6 text-white shadow-lg"><p class="text-xs sm:text-sm opacity-80">Einnahmen</p><p class="text-lg sm:text-2xl font-bold mt-1">{formatCurrency(monthlyBalance.income)}</p></div>
      <div class="bg-gradient-to-br from-red-400 to-red-600 rounded-2xl p-4 sm:p-6 text-white shadow-lg"><p class="text-xs sm:text-sm opacity-80">Ausgaben</p><p class="text-lg sm:text-2xl font-bold mt-1">{formatCurrency(monthlyBalance.expenses)}</p></div>
      <div class="bg-gradient-to-br {monthlyBalance.balance >= 0 ? 'from-blue-400 to-blue-600' : 'from-orange-400 to-orange-600'} rounded-2xl p-4 sm:p-6 text-white shadow-lg"><p class="text-xs sm:text-sm opacity-80">Saldo</p><p class="text-lg sm:text-2xl font-bold mt-1">{formatCurrency(monthlyBalance.balance)}</p></div>
      <div class="bg-gradient-to-br from-purple-400 to-indigo-600 rounded-2xl p-4 sm:p-6 text-white shadow-lg"><p class="text-xs sm:text-sm opacity-80">Vermögen</p><p class="text-lg sm:text-2xl font-bold mt-1">{formatCurrency(netWorth.totalValue)}</p></div>
    </div>
    ...
    <div class="hidden lg:block space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <TaxExport {transactions} />
        <BillDetection {transactions} />
      </div>
      <FinancialGoals {savingsGoals} />
    ...
    <div class="lg:hidden">
      {#if activeTab === 'overview'}<DashboardCharts {monthlyOverview} {budgetProgress} {transactions} />
      {:else if activeTab === 'networth'}<NetWorthDashboard />
  ...
  <footer class="mt-12 py-6 border-t border-gray-200 dark:border-gray-700 hidden lg:block"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-sm text-gray-500 dark:text-gray-400"><p>💰 Money MVP — Dein smarter Finanz-Tracker</p></div></footer>
</div>
